...
export default function Home() {
    ...
    return (
        <div className={"flex min-h-screen w-full relative"}>
            ...
            {(isVisible) ? (
                <div id={"wrapper"} onClick={handleClose} className={"absolute z-20 fixed inset-0 bg-black bg-opacity-25 backdrop-blur-sm flex-col flex justify-center items-center"}>
                    <div className={"w-1/3 min-w-min h-1/3 min-h-min flex flex-col"}>
                        <button onClick={() => setIsVisible(false)} className={"text-white text-xl place-self-end"}>X</button>
                        <div className={"bg-white flex-col rounded-3xl p-5"}>
                            <form className={"flex flex-col items-start"}>
                                <div className={"p-2 rounded-3xl flex flex-row items-center mb-3 border-b-2 border-b-sky-400 w-full"}>
                                    ...
                                    <input type={"text"} placeholder={"Ex: Kioi1-3 Chioda,Tokyo"}
                                           className={"text-lg outline-none flex-1 p-2 hover:bg-gray-100"}/>
                                </div>
                                <div className={"p-2 rounded-3xl flex flex-row items-center mb-3 border-b-2 border-b-sky-400 w-full"}>
                                    <IoText className={"text-gray-400 mr-2"} />
                                    <textarea placeholder={"Ex: It's dangerous!"}
                                           className={"text-lg outline-none flex-1 p-2 hover:bg-gray-100"}/>
                                </div>
                                <div className={"p-2 aspect-video border-dashed border-2 items-center border-sky-600 rounded-2xl bg-gray-100 hover:bg-gray-200"}>
